...
relationships <- relationships %>% 
  transmute(
    p_column, directory, is_kin, family_folder, kin_pairs = row_num
  )
```
...
if(!file.exists('all_relationships.rda')) {
      ...
      function(x) {
        tibble(directory = x$directory,
          ...
          transmute(
            row_id     = x$row_id,
            filenames,
            kin_pairs = x$kin_pairs,
            family_folder = x$family_folder,
            is_kin = x$is_kin,
            p_column = x$p_column
          )
      }
 ...
 all_relationships <- readRDS('all_relationships.rda') 
}
...
order_parent_shuffle <- 
  ...
  transmute(parent_num, 
            parent_file, 
            family_distinction_parent = family_folder) %>% 
  arrange(parent_num) %>% 
  select(-parent_num)
...
order_child_shuffle <- 
  ...
  select(child_num, 
         child_file, 
         family_distinction_child = family_folder, 
         is_kin) %>% 
  arrange(child_num) %>% 
  select(-child_num)

n_positive <- nrow(all_relationships)
training_data <- bind_cols(order_parent_shuffle, order_child_shuffle) %>% 
    ...
    mutate(row_num = row_number()) %>% 
    filter(row_num %in% 1:n_positive) %>% 
    select(-row_num) %>% 
